...
with open('intracity_fare_train.csv') as csvfile:
	...
	with open('intracity_fare_test.csv') as TestData:
		...
		reg =  XGBRegressor(max_depth=7,learning_rate=0.04)	#learning rate makes the model robust by shrinking weights on each step
		reg.fit(X ,y)								#higher depth allows the model to learn relations specific to particular sample
		prediction = reg.predict(data1)
		
		with open('result_fare.csv',"w") as f:
